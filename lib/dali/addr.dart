...
class DaliAddr {
  ...
  Future<bool> compareSingleAddress(int typ, int addr) async {
    try {
      ...
      if (await base.compareAddress()) return true;
    } on DaliDeviceNoResponseException {
      return false; // 设备无响应 => 不匹配
    }
    DaliLog.instance.debugLog('ERROR [compareSingleAddress]: unexpected exception');
    return false;
  }
  ...
  /// 二分法 to compare address, not working
  ...
  Future<void> allocateAllAddr([int? ads]) async {
    ...
    for (int i = 0; i <= 80; i++) {
      ...
      ad++;
      if (ad > 63) {
        log.errorLog('compare process aborted, address out of range');
        isAllocAddr = false;
        break;
      }
      if (i == 80) {
